...
struct ContentView: View {
    ...
    var body: some View {
        TabView {
            ...
            Tab("Emulator", systemImage: "display") {
                NavigationStack {
                    EmulatorView(memory: (try? Assembler.compileBytes(input: savedCode)) ?? [])
                }
            }
        }
    ...
    private static let snakeGame: String = """
        ...
        spinWheels:
          ; Calculate delay: 256 - snakeLength * factor
          lda #0             ; start with 256 (0 wraps to 256 in DEX loop)
          sec
          sbc snakeLength    ; subtract snake length
          sbc snakeLength    ; subtract again (each segment adds ~10 cycles to updateSnake)
          tax
        spinloop:
          nop
          nop
